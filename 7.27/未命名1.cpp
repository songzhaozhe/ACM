...
void mark(int f[],char fla[],int qq[])
{
	int num,i=0;
	do{
		num=f[p[i][0]]*4+f[p[i][1]]*3+f[p[i][2]]*2+f[p[i][3]];
		fla[num]++;
		qq[i]=num;
		i++;
	}while(i<=23);	
...
//num=(f[p[i][0]]<<2)+(f[p[i][1]]<<1|1)+(f[p[i][2]]<<1)+f[p[i][3]];
