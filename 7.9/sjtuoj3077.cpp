...
int s,f;
LL dp[maxn][2];
char buffer[maxn*10+1000];
int first[maxn],next[maxn],rear,in[maxn],edgerear,u[maxn],v[maxn];
...
//	dp[i][0]=dp[i][1]=0;
	...
	for(e=first[i];e!=-1;e=next[e])
	{
		int to=v[e];
		dfs(to);
		if(dp[to][0]<dp[to][1]||(dp[to][0]==dp[to][1]&&t[to][0].get()<t[to][1].get()))
		{
			dp[i][0]+=dp[to][0];
		...
		//family
		if(dp[to][0]-s<dp[to][1]||(dp[to][0]-s==dp[to][1]&&t[to][0].get()-1<t[to][1].get()))
		{
			dp[i][1]+=dp[to][0]-s;
